        ...
        (err, results) => {
          ...
          Promise.all(addOrderItems)
            .then(() => {
              ...
              Promise.all(reduceStockPromises)
                .then(() => {
                  User.updatePoints(
                    userId,
                    -totalPoints,
                    (err, updateResult) => {
                      if (err)
                        return res
                          .status(500)
                          .send('Error updating user points');
                      Cart.clearCart(userId, (err, clearResult) => {
                        if (err) return res.status(500).send(err);
                        res
                          .status(200)
                          .send(
                            'Order created, stock updated, and cart cleared'
                          );
                      });
                    }
                  );
...
exports.getOrderHistory = (req, res) => {
  ...
  Order.getOrderHistoryByUser(userId, (err, orderHistory) => {
    ...
    const formattedHistory = orderHistory.map((order) => ({
      description: moment(order.created_at).format('dddd Do MMMM YYYY'),
